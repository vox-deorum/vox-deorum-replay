(function () {
   ...
   */
  function extractMapTerrain(body, mapPos, width, height, ownedPlots) {
      ...
      const plausibleHead = (start, layout) => {
      ...
      if (full.length > 0) {
          ...
          if (ownedPlots !== undefined) {
              ...
              if (ownersAgree.length > 0)
                  finalists = ownersAgree;
          }
          const headsPlausible = finalists.filter(f => plausibleHead(f.start, f.layout));
          if (headsPlausible.length > 0)
              finalists = headsPlausible;
  ...
  class SaveParser extends BaseParser {
      ...
      async parseReplay() {
          ...
          if (mapDims.mapPos >= 0 && mapDims.width > 0 && mapDims.height > 0) {
              terrain = extractMapTerrain(this.decompressed, mapDims.mapPos, mapDims.width, mapDims.height, mapDims.header ? mapDims.header.ownedPlots : undefined);
